...
- **`NEXT_PUBLIC_SITE_URL`**: set to your public origin (e.g. `https://persiana.store`, no trailing slash) on the Cloud Run service and as a Docker build arg so metadata and client bundles use the right host (see root `cloudbuild.yaml` substitution `_NEXT_PUBLIC_SITE_URL`). Runtime-only env fixes server metadata after the layout change; rebuilding with the build-arg still helps for any client-inlined `NEXT_PUBLIC_*` usage.
