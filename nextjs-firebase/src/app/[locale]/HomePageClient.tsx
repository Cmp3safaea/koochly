...
export default function HomePageClient({
  initialCities: initialCitiesProp = [],
  initialEvents: initialEventsProp = [],
}: HomePageClientProps) {
  const { t, locale } = useI18n();
  const docTheme = useDocumentTheme();
  const loc = useLocalizedHref();
  const [cities, setCities] = useState<City[]>(() => initialCitiesProp);
  const [citiesStatus, setCitiesStatus] = useState<string | null>(null);
  const [openCountry, setOpenCountry] = useState<string | null>(null);
  const [hasLoaded, setHasLoaded] = useState(() => initialCitiesProp.length > 0);
  const [search, setSearch] = useState("");
  const [user, setUser] = useState<User | null>(null);
  const [profileCity, setProfileCity] = useState("");
  const [events, setEvents] = useState<EventItem[]>(() => initialEventsProp);
  ...
  useEffect(() => {
    ...
    if (initialCitiesProp.length > 0) {
      setHasLoaded(true);
      setCitiesStatus(null);
    } else {
      void refresh(false);
    }
  ...
  }, [t, initialCitiesProp.length]);

  useEffect(() => {
    if (initialEventsProp.length > 0) {
      setEventsStatus(null);
      return;
    }
    let cancelled = false;
